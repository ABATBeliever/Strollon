...
class VerticalTabBrowser(QMainWindow):
    ...
    def __init__(self):
        ...
        # 「リンクを保存」等、明示的なダウンロード操作が行われたページを
        # 一時的に記録する（このページからの次のダウンロードはPDFビューアに
        # 差し替えず、そのまま通常のダウンロードとして扱う）。
        # ページオブジェクトの id() はGC後に再利用されるため、ページに
        # 直接属性を付与する方式を使い、id()依存を排除する。
        self._explicit_save_pending: set = set()  # page._strollon_save_token の集合
    ...
    def _should_open_pdf_in_viewer(self, download) -> bool:
        ...
        page = download.page()
        if page is not None:
            token = getattr(page, "_strollon_save_token", None)
            if token and token in self._explicit_save_pending:
                self._explicit_save_pending.discard(token)
                try:
                    del page._strollon_save_token
                except Exception:
                    pass
                return False
    ...
    def _handle_pdf_download(self, download):
        ...
        # exists() と stat() の間にファイルが消える可能性（TOCTOU）があるため
        # stat() を直接 try/except で囲み、消えていたら通常のダウンロードへ fallback する
        _use_cache = False
        try:
            if cache_path.stat().st_size > 0:
                _use_cache = True
        except OSError:
            pass  # キャッシュファイルが存在しないか消えた → 通常ダウンロードへ
    ...
    def _hook_explicit_save_actions(self, page):
        ...
        ページオブジェクトの id() は GC 後に再利用されるため、ページに直接
        トークン属性を付与して管理し id() 依存を排除する。"""
        try:
            from PySide6.QtWebEngineCore import QWebEnginePage as _Page
            for action_enum in (
    ...
    def _mark_explicit_save(self, page):
        """ページにUUIDトークンを付与してexplicit_saveセットに登録する。
        id() の再利用リスクを避けるためオブジェクト本体にトークンを保持させる。"""
        import uuid as _uuid
        token = _uuid.uuid4().hex
        try:
            page._strollon_save_token = token
        except Exception:
            return
        self._explicit_save_pending.add(token)
        # downloadRequested が発火しなかった場合に備えてタイムアウトで自動解除する
        QTimer.singleShot(5000, lambda: self._explicit_save_pending.discard(token))
